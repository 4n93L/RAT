...
parser.add_argument(
    "-i", "--ip", dest="IP", default="", help="the ip address of the server."
)
parser.add_argument(
    "-p", "--port", dest="PORT", default=4444, help="the port number of the server."
)
parser.add_argument(
    "-t",
    "--time",
    dest="TIME",
    default=60,
    help="the number of seconds of automatic message sending to check if the connection is alive or dead.",
)
parser.add_argument(
    "-pa",
    "--password",
    dest="PASSWORD",
    default="ANG3LTHEBOSS",
    help="The password to generate the key to encrypt and decrypt the data. The default password is 'ANG3LTHEBOSS'.",
)
parser.add_argument(
    "-c",
    "--clean",
    ...
    action="store_true",
    default=False,
    help="cleans all the data in the database.",
)
parser.add_argument(
    "-d",
    "--display",
    action="store_false",
    default=True,
    help="disables the display of client information (Logoff and incoming connection).",
)
...
try:
    ...
    try:
        ...
        printColor("successfully", "By @ang3L")
