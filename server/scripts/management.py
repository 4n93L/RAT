...
class CheckConn:
    ...
    def sendsafe(
        self, nb_session, sock, payload, display=True
    ):  # display = Avoids display bugs (useful in broadcast mode)
        """
        ...
        try:
            ...
            if Handler.status_connection_display and display:
                printColor(
                    "error",
                    "[-] The connection to the client was cut {}:{}.\n".format(
                        Handler.dict_conn[nb_session][NB_IP],
                        Handler.dict_conn[nb_session][NB_PORT],
                    ),
                )
    ...
    def recvsafe(
        self, sock, buffer
    ):  # Receives a single secure data (no need for "\r\n") to finish the connection
        result = ""
        sock.settimeout(SOCK_TIMEOUT)
        try:
            data_tmp = sock.recv(buffer).decode("utf-16-le", "replace")
    ...
    def recvcommand(
        self, sock, buffer
    ):  # The decryption of xor is automatic on this method. Does not send data displays the data in real time.
        ...
        printColor(
            "information",
            "[?] the command can take up to {} seconds before triggering an exception (timeout).\n".format(
                timeout
            ),
        )
        ...
        for i in range(len(list_request) - 1):  # -1 for ingore last request (\r\n)
            printColor(
                "successfully",
                XOREncryption(
                    list_request[i].decode("utf-16-le", "replace"), Handler.PBKDF2_Key
                ),
            )
