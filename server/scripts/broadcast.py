...
class Broadcast:
    ...
    def __init__(self):
        Broadcast.checker = (
            True  # Resets the class attribute each time the constructor is called.
        )
    ...
    def aSingleRunMod(self):  # broadcast
        ...
        for key in copy_dict_conn.keys():
            ...
            for key_of_dict_sorts in dict_sorts:
                if (
                    dict_sorts[key_of_dict_sorts][NB_TOKEN]
                    == copy_dict_conn[key][NB_TOKEN]
                ):
                    already_dict = True
                else:
                    pass
            ...
            if (
                not (already_dict) and copy_dict_conn[key][NB_ALIVE]
            ):  # Value already existing and now online
            ...
            for key in dict_sorts.keys():
                if dict_sorts[key][NB_ALIVE]:
                    if CheckConn().sendsafe(
                        key, dict_sorts[key][NB_SOCKET], request, False
                    ):
                        printColor(
                            "information",
                            "[+] The persistence was sent to the client number {} {}:{}".format(
                                dict_sorts[key][NB_SESSION],
                                dict_sorts[key][NB_IP],
                                dict_sorts[key][NB_PORT],
                            ),
                        )
                        ...
                        printColor(
                            "information",
                            "[-] The persistence could not be sent to: {}:{}".format(
                                dict_sorts[key][NB_IP], dict_sorts[key][NB_PORT]
                            ),
                        )
                else:
                    pass
            ...
            for key in dict_sorts.keys():
                if dict_sorts[key][NB_ALIVE]:
                    if CheckConn().sendsafe(
                        ...
                        printColor(
                            "information",
                            "[+] The command could not be sent to: {}:{}".format(
                                dict_sorts[key][NB_IP], dict_sorts[key][NB_PORT]
                            ),
                        )
                else:
    ...
    def broadcast_to_all_clients(self, data, whitout_MOD_ALL=False):
        ...
        for key in Handler.dict_conn.keys():
            if Handler.dict_conn[key][NB_ALIVE]:
                if CheckConn().sendsafe(
                    key, Handler.dict_conn[key][NB_SOCKET], request
                ):
                    printColor(
                        "information",
                        "[+] command sent to: {}:{}".format(
