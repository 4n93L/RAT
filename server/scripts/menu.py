...
""",
    ...
    def selectTarget(self, target):
        ...
        if target in Handler.dict_conn.keys():
            ...
            printColor(
                "information",
                "[+] Selected session: {}".format(
                    Handler.dict_conn[target][NB_SESSION]
                ),
            )
            if Handler.dict_conn[target][NB_ALIVE]:
                Handler.dict_conn[target][NB_SELECT] = True
                session = Session(
                    Handler.dict_conn[target][NB_SOCKET],
                    Handler.dict_conn[target][NB_IP],
                    Handler.dict_conn[target][NB_PORT],
                    target,
            ...
            printColor(
                "information", "[-] No target found. Please enter a valid target.\n"
            )
            ...
            for i in range(0, len(choice)):
                if choice[i] == "--help" or choice[i] == "-h":
                    ...
                    try:
                        self.selectTarget(
                            int(choice[i + 1])
                        )  # int car -t 10 <- 10 doit ^etre un int
                    except IndexError:
                        printColor(
                            "information",
                            "[-] No target found. Please enter a valid target.\n",
                        )
